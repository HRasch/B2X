...
describe('CMS Store', () => {
  ...
  describe('Fetch Pages', () => {
    ...
    it('should set loading state', async () => {
      vi.mocked(cmsApi.getPages).mockImplementation(
        () => new Promise(resolve => {
          setTimeout(() => resolve({ items: [], total: 0, page: 1, pageSize: 10, totalPages: 0 }), 100)
        })
      )
  ...
  describe('Fetch Templates', () => {
    it('should fetch templates', async () => {
      const mockTemplates = [
        { id: '1', name: 'Hero Template', description: 'Hero section', blocks: [], createdAt: new Date(), updatedAt: new Date(), tenantId: 'tenant-1' },
      ]
