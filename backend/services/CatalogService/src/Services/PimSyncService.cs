...
public class PimSyncService : IPimSyncService
{
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            // Convert batch to NDJSON format for bulk API
            ...
            // Use raw request for bulk indexing (more efficient)
            var bulkRequest = new Elastic.Clients.Elasticsearch.Core.BulkRequest
            {
                Operations = new List<Elastic.Clients.Elasticsearch.BulkOperation>()
            };

            // Execute bulk index
            _logger.LogDebug("Executing bulk index with {OperationCount} operations", batch.Count / 2);

            // Note: For production, use proper bulk API with typed requests
