...
namespace B2X.CLI.Administration.Commands.AuthCommands;
...
public static class LoginCommand
{
    public static Command Create()
    {
        ...
        var saveOption = new Option<bool>(
            ...
            description: "Save token to B2X_TOKEN environment variable");
    ...
    private static async Task ExecuteAsync(string email, string password, bool save)
    {
        ...
        try
        {
            ...
            if (response != null)
            {
                ...
                if (save)
                {
                    Environment.SetEnvironmentVariable("B2X_TOKEN", response.Token, EnvironmentVariableTarget.User);
                    console.Success("Token saved to B2X_TOKEN");
                }
            }
        }
        catch (Exception ex)
