...
public static class BenchmarkCommand
{
    ...
    private static async Task ExecuteAsync(string service, string tenant, int duration, int concurrency, string output, bool save)
    {
        ...
        try
        {
            console.Header("B2Connect Service Benchmarking");
            ...
            if (!services.Any())
            {
                console.Warning("No services configured. Use 'b2connect-admin config set service.name url' to configure services.");
                return;
            }
