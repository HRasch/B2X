# B2Connect Benutzerhandbuch - Deutsch

Willkommen zur B2Connect-Benutzerdokumentation! Finden Sie Antworten auf Ihre Fragen zur Kontoverwaltung, Sicherheit und den ersten Schritten.

---

## 📚 Verfügbare Anleitungen

### Erste Schritte
- **[Erste Schritte mit B2Connect](./getting-started.md)** - Zum ersten Mal auf B2Connect? Erfahren Sie, wie Sie ein Konto erstellen und mit dem Einkaufen beginnen
  - Was ist B2Connect?
  - Warum ein Konto erstellen
  - Kontoerstellung
  - Erste Anmeldung
  - Nächste Schritte

### Authentifizierung und Konten
- **[Wie Sie sich anmelden](./login.md)** - Greifen Sie auf Ihr Konto auf jedem Gerät zu
  - Anmeldung auf dem Desktop
  - Mobile Anmeldung (iPhone, Android)
  - Angemeldet bleiben
  - Fehlerbehebung bei Anmeldeproblemen

- **[Wie Sie Ihr Passwort zurücksetzen](./reset-password.md)** - Passwort vergessen?
  - Schnelle Schritte zum Zurücksetzen des Passworts
  - Erstellen Sie sichere Passwörter
  - Fehlerbehebung bei E-Mail
  - Häufig gestellte Fragen

### Sicherheit
- **[Kontotipps zur Sicherheit](./account-security.md)** - Halten Sie Ihr Konto sicher
  - Wesentliche Sicherheitsregeln
  - Sichere Passwörter
  - Passwortfreigabe (niemals!)
  - Sicherheit auf öffentlichen Computern
  - Zwei-Faktor-Authentifizierung (2FA)
  - Maßnahmen bei Sicherheitsproblemen

---

## 🔍 Schnelle Suche

**Ich möchte...**

- **Ein Konto erstellen** → [Erste Schritte](./getting-started.md)
- **Mich bei meinem Konto anmelden** → [Wie Sie sich anmelden](./login.md)
- **Mein Passwort zurücksetzen** → [Wie Sie Ihr Passwort zurücksetzen](./reset-password.md)
- **2FA für Sicherheit aktivieren** → [Kontotipps zur Sicherheit](./account-security.md)
- **Erfahren Sie mehr über sichere Passwörter** → [Kontotipps zur Sicherheit](./account-security.md#passwort-tipps-und-tricks)
- **Verdächtige Aktivität melden** → [Kontotipps zur Sicherheit](./account-security.md#was-ist-wenn)

---

## ❓ Häufig gestellte Fragen

### Kontoerstellung und Einrichtung
**F: Ist es kostenlos, ein B2Connect-Konto zu erstellen?**  
A: Ja! Die Kontoerstellung ist völlig kostenlos.

**F: Brauche ich ein Konto zum Durchsuchen von Produkten?**  
A: Sie können ohne Konto durchsuchen, aber Sie benötigen eines, um Bestellungen aufzugeben.

**F: Wie erstelle ich ein Konto?**  
A: Siehe [Erste Schritte](./getting-started.md) für Schritt-für-Schritt-Anweisungen.
### Anmeldeprobleme
**F: Ich habe mein Passwort vergessen, was soll ich tun?**  
A: Gehen Sie zu [Wie man sein Passwort zurückgesetzt](./reset-password.md).
Sie Ihr Passwort zurücksetzen](./reset-password.md).**F: Warum sagt es "Falsche E-Mail oder falsches Passwort"?**  
A: Überprüfen Sie die Schreibweise Ihrer E-Mail und Ihres Passworts. Denken Sie daran, dass Passwörter zwischen Groß- und Kleinschreibung unterscheiden!

**F: Kann ich mich auf mehreren Geräten anmelden?**  
A: Ja! Sie können gleichzeitig auf Telefon, Computer und Tablet angemeldet sein.

### Sicherheit
**F: Sind meine Daten auf B2Connect sicher?**  
A: Ja! Wir verwenden Verschlüsselung und Sicherheitspraktiken nach Branchenstandard.

**F: Was ist Zwei-Faktor-Authentifizierung?**  
A: Es ist zusätzliche Sicherheit, die Ihr Telefon plus Ihr Passwort erfordert. [Erfahren Sie mehr](./account-security.md#zwei-faktor-authentifizierung-2fa).

**F: Sollte ich überall das gleiche Passwort verwenden?**  
A: Nein! Verwenden Sie ein eindeutiges Passwort für B2Connect. [Sehen Sie, warum](./account-security.md).

---
## 🆘 Benötigen Sie Hilfe?

### Support kontaktieren
Wenn Sie eine Frage haben, die hier nicht beantwortet wird:
- **E-Mail**: support@b2connect.com
- **Live-Chat**: Auf der B2Connect-Website verfügbar (9 Uhr - 17 Uhr, 5 Tage pro Woche)
- **Telefon**: +49 (0)800 123 4567

### Ein Problem melden
- Verdächtige Aktivität auf Ihrem Konto?
- Nicht autorisierte Gebühren?
- Konto gehackt?

[Kontaktieren Sie den Support sofort](../README.md)

---

## 🌍 Verfügbare Sprachen

- 🇩🇪 **Deutsch** (Sie sind hier)
- 🇬🇧 [**English** (Englisch)](../en/)

---

## 📋 Vollständiger Anleitung-Index

1. Erste Schritte mit B2Connect
2. Wie man sich anmeldet
3. Wie man sein Passwort zurückgesetzt
4. Kontotipps zur Sicherheit

**Demnächst:**
- Wie man eine Bestellung aufgibt
- Verfolgung Ihrer Bestellungen
- Rückgaben & Erstattungen
- Zahlungsmethoden
- Kontoeinstellungen

---

## Zuletzt aktualisiert

**Version**: 1.0  
**Zuletzt aktualisiert**: 29. Dezember 2025  
**Sprache**: Deutsch  

---

**Haben Sie Feedback?** Wir würden gerne von Ihnen hören!  
Helfen Sie uns, diese Anleitungen zu verbessern, indem Sie uns unter support@b2connect.com kontaktieren

