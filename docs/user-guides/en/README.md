# B2Connect User Guides - English

Welcome to the B2Connect user documentation! Find answers to your questions about account management, security, and getting started.

---

## 📚 Available Guides

### Getting Started
- **[Getting Started](./getting-started.md)** - First time on B2Connect? Learn how to create an account and start shopping
  - What is B2Connect?
  - Creating your account
  - First login
  - Next steps

### Authentication & Accounts
- **[How to Log In](./login.md)** - Access your account on any device
  - Desktop login
  - Mobile login (iPhone, Android)
  - Staying logged in
  - Troubleshooting login issues

- **[How to Reset Your Password](./reset-password.md)** - Forgot your password?
  - Quick password reset steps
  - Creating strong passwords
  - Email troubleshooting
  - Frequently asked questions

### Security
- **[Account Security Tips](./account-security.md)** - Keep your account safe
  - Essential security rules
  - Strong passwords
  - Password sharing (never!)
  - Public computer safety
  - Two-factor authentication (2FA)
  - What to do if compromised

---

## 🔍 Quick Search

**I want to...**

- **Create an account** → [Getting Started](./getting-started.md)
- **Log in to my account** → [How to Log In](./login.md)
- **Reset my password** → [How to Reset Your Password](./reset-password.md)
- **Enable 2FA for security** → [Account Security Tips](./account-security.md)
- **Learn about strong passwords** → [Account Security Tips](./account-security.md#password-tips--tricks)
- **Report suspicious activity** → [Account Security Tips](./account-security.md#what-to-do-if)

---

## ❓ Frequently Asked Questions

### Account Creation & Setup
**Q: Is it free to create a B2Connect account?**  
A: Yes! Creating an account is completely free.

**Q: Do I need an account to browse products?**  
A: You can browse without an account, but you need one to place orders.

**Q: How do I create an account?**  
A: See [Getting Started](./getting-started.md) for step-by-step instructions.

### Login Issues
**Q: I forgot my password, what do I do?**  
A: Go to [How to Reset Your Password](./reset-password.md).

**Q: Why does it say "wrong email or password"?**  
A: Check the spelling of your email and password. Remember, passwords are case-sensitive!

**Q: Can I log in on multiple devices?**  
A: Yes! You can be logged in on your phone, computer, and tablet at the same time.

### Security
**Q: Is my data safe on B2Connect?**  
A: Yes! We use industry-standard encryption and security practices.

**Q: What is 2-factor authentication?**  
A: It's extra security that requires your phone plus your password. [Learn more](./account-security.md#two-factor-authentication-2fa).

**Q: Should I use the same password everywhere?**  
A: No! Use a unique password for B2Connect. [See why](./account-security.md).

---

## 🆘 Need Help?

### Contact Support
If you have a question not answered here:
- **Email**: support@b2connect.com
- **Live Chat**: Available on B2Connect website (9 AM - 5 PM, 5 days/week)
- **Phone**: +49 (0)800 123 4567

### Report a Problem
- Suspicious activity on your account?
- Unauthorized charges?
- Account hacked?

[Contact Support immediately](../README.md)

---

## 🌍 Available Languages

- 🇬🇧 **English** (you are here)
- 🇩🇪 [**Deutsch** (German)](../de/)

---

## 📋 Complete Guide Index

1. Getting Started with B2Connect
2. How to Log In
3. How to Reset Your Password
4. Account Security Tips

**Coming Soon:**
- How to Place an Order
- Tracking Your Orders
- Returns & Refunds
- Payment Methods
- Account Settings

---

## Last Updated

**Version**: 1.0  
**Last Updated**: 29 December 2025  
**Language**: English  

---

**Have feedback?** We'd love to hear from you!  
Help us improve these guides by contacting support@b2connect.com

