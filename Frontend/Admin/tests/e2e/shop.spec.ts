...
test.describe('Catalog API Tests', () => {
  test('should access Products endpoint with tenant header', async ({ page }) => {
    ...
    expect(response.status === 200 || response.status === 404 || response.error !== undefined).toBe(
      true
    );
  });

  test('should access Categories endpoint with tenant header', async ({ page }) => {
    const response = await page.evaluate(
      async ({ apiBase, tenantId }) => {
        try {
