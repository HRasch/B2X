...
export const adminService = {
  ...
  async createAdmin(admin: Omit<Administrator, 'id' | 'createdAt' | 'updatedAt'>, tenantId: string): Promise<Administrator> {
  ...
  async updateAdmin(id: string, admin: Partial<Administrator>, tenantId: string): Promise<Administrator> {
  ...
  async inviteAdmin(email: string, role: string, tenantId: string): Promise<{ invitationId: string }> {
    const response = await apiClient.post(`/tenant/admins/invite`, { email, role }, {
      headers: { 'X-Tenant-ID': tenantId }
    })
    return response.data.data
  }
