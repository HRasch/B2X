<template>
  <div class="customer-type-selection">
    <div class="container">
      <div class="header">
        <h1>{{ t('customerTypeSelection.title') }}</h1>
        <p class="subtitle">{{ t('customerTypeSelection.subtitle') }}</p>
      </div>
      ...
      <div class="options-grid">
        ...
        <button
          ...
          :aria-label="t('customerTypeSelection.private.ariaLabel')"
          :aria-pressed="selectedType === 'private'"
        >
          <div class="icon">👤</div>
          <h2>{{ t('customerTypeSelection.private.title') }}</h2>
          <p class="description">{{ t('customerTypeSelection.private.description') }}</p>
          <p class="details">{{ t('customerTypeSelection.private.details') }}</p>
        </button>
        ...
        <button
          ...
          :aria-label="t('customerTypeSelection.business.ariaLabel')"
          :aria-pressed="selectedType === 'business'"
        >
          <div class="icon">🏢</div>
          <h2>{{ t('customerTypeSelection.business.title') }}</h2>
          <p class="description">{{ t('customerTypeSelection.business.description') }}</p>
          <p class="details">{{ t('customerTypeSelection.business.details') }}</p>
        </button>
      </div>

      <!-- Action Buttons -->
      <div class="actions">
        <button class="btn btn-primary" :disabled="!selectedType" @click="continueRegistration">
          {{ t('customerTypeSelection.actions.continue') }}
        </button>
      </div>

      <div class="login-link">
        <p>
          {{ t('customerTypeSelection.login.prompt') }}
          <router-link to="/login" class="link">{{
            t('customerTypeSelection.login.link')
          }}</router-link>
        </p>
      </div>
    </div>
