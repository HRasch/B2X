...
// Test configuration
const API_BASE_URL =
  process.env.VITE_API_GATEWAY_URL || "http://localhost:8000";
...
describe("ProductService Integration Tests", () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      console.warn(
        `Backend not available at ${API_BASE_URL} - skipping integration tests`,
      );
        ...
        try {
          ...
          if (axios.isAxiosError(error)) {
            ...
            if (error.response?.status === 404) {
              throw new Error(
                "Route /api/catalog/products not found - check YARP config",
              );
            }
      ...
      async () => {
        try {
          ...
          if (axios.isAxiosError(error)) {
            if (error.response?.status === 404) {
              throw new Error(
                "Route /api/v1/products not found - check Catalog service routing",
              );
            }
            expect([401, 403, 500]).toContain(error.response?.status);
          } else {
      ...
      async () => {
        try {
          ...
          if (axios.isAxiosError(error)) {
            if (error.response?.status === 404) {
              console.warn(
                "Route /api/v2/products not found - may not be in dev mode",
              );
            }
      ...
      async () => {
        try {
          ...
          if (axios.isAxiosError(error) && error.response?.status === 401) {
            console.warn(
              "Auth required for /api/catalog/products - skipping structure test",
            );
            return;
          }
          throw error;
        ...
        try {
          ...
          if (axios.isAxiosError(error)) {
            ...
            if (error.response?.status === 400) {
              console.warn(
                "Bad request - check API contract:",
                error.response.data,
              );
            }
          }
