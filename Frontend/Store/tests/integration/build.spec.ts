...
describe('Build Integration - Vite Configuration', () => {
  ...
  it('should have valid tailwind.config.ts', () => {
    ...
    expect(existsSync(configPath), 'tailwind.config.ts should exist').toBe(true);
...
describe('Build Integration - Dependencies', () => {
  it('should have compatible Tailwind and DaisyUI versions', () => {
    ...
    const isTailwindV4 = tailwindVersion.includes('4.') || tailwindVersion.includes('^4');
    ...
    if (isTailwindV4) {
      ...
      expect(hasPostcssPlugin, '@tailwindcss/postcss required for Tailwind v4').toBeTruthy();
    }
  });
  ...
  it('should have all required dependencies', () => {
    ...
    const requiredDeps = ['vue', 'vue-router', 'pinia', 'tailwindcss', 'daisyui'];
    ...
    for (const dep of requiredDeps) {
      expect(allDeps[dep], `Dependency "${dep}" should be installed`).toBeTruthy();
    }
  });
});
