...
test.describe("Build Health - Vite Compilation", () => {
  test("should load app without console errors", async ({
    page,
  }: {
    page: Page;
  }) => {
...
test.describe("Build Health - Tailwind CSS Classes", () => {
  test("should render Tailwind utility classes correctly", async ({
    page,
  }: {
    page: Page;
  }) => {
    ...
    for (const testCase of testCases) {
      ...
      expect(result, `Tailwind class "${testCase.class}" should work`).toBe(
        testCase.expected
      );
    }
  });

  test("should render DaisyUI component classes correctly", async ({
    page,
  }: {
    page: Page;
  }) => {
    ...
    for (const className of daisyClasses) {
      ...
      expect(
        hasStyles,
        `DaisyUI class "${className}" should apply styles`
      ).toBe(true);
    }
  });

  test("should render DaisyUI color semantic classes", async ({
    page,
  }: {
    page: Page;
  }) => {
    ...
    for (const className of colorClasses) {
      ...
      expect(hasColor, `DaisyUI class "${className}" should apply color`).toBe(
        true
      );
    }
  });
...
test.describe("Build Health - Page Load Verification", () => {
  ...
  for (const pageInfo of criticalPages) {
    test(`should load ${pageInfo.name} page without errors`, async ({
      ...
      expect(
        response?.status(),
        `${pageInfo.name} should return 200 OK`
      ).toBeLessThan(400);
      ...
      expect(
        criticalErrors,
        `${pageInfo.name} should have no critical errors`
      ).toHaveLength(0);
    });
  }
});
...
test.describe("Build Health - Asset Loading", () => {
  ...
  test("should have working Vue app mount", async ({
    page,
  }: {
    page: Page;
  }) => {
