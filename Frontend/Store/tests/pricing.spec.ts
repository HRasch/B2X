...
test.describe("PAngV Compliance - Price Transparency", () => {
  ...
  test("Cart summary displays correct VAT calculation", async ({ page }) => {
    ...
    const subtotal = parseFloat(
      subtotalText?.match(/(\d+,\d{2})/)?.[1]?.replace(",", ".") || "0"
    );
    const tax = parseFloat(
      taxText?.match(/(\d+,\d{2})/)?.[1]?.replace(",", ".") || "0"
    );
    const total = parseFloat(
      totalText?.match(/(\d+,\d{2})/)?.[1]?.replace(",", ".") || "0"
    );
  ...
  test("Price format uses German locale (comma separator)", async ({
    page,
  }) => {
    await page.goto("http://localhost:5173/shop");
    await page.click(".add-to-cart-btn");
...
test.describe("Checkout Form Validation", () => {
  ...
  test("All required fields prevent submission when empty", async ({
    page,
  }) => {
    await page.goto("http://localhost:5173/shop");
    await page.click(".add-to-cart-btn");
