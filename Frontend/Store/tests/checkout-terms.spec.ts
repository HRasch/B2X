...
test.describe("Checkout Terms & Conditions - P0.6-US-005", () => {
  ...
  test("Checkout displays terms acceptance step", async ({ page }) => {
    ...
    await expect(
      page.locator("text=Allgemeinen Geschäftsbedingungen")
    ).toBeVisible();
  ...
  test("View Terms & Conditions modal", async ({ page }) => {
    ...
    const modal = page.locator(
      "h3:has-text('Allgemeine Geschäftsbedingungen')"
    );
    await expect(modal).toBeVisible();

    // Verify content sections
    await expect(
      page.locator("h4:has-text('Allgemeine Bestimmungen')")
    ).toBeVisible();
  ...
  test("Accessibility: WCAG 2.1 AA compliance", async ({ page }) => {
    ...
    await expect(
      page.locator('label[for="withdrawal-checkbox"]')
    ).toBeVisible();

    // Document links are navigable
    const termsLink = page
      .locator("text=Allgemeinen Geschäftsbedingungen")
      .first();
    ...
    await expect(
      page.locator("h3:has-text('Allgemeine Geschäftsbedingungen')")
    ).toBeVisible();
    await page.keyboard.press("Escape");
    await expect(
      page.locator("h3:has-text('Allgemeine Geschäftsbedingungen')")
  ...
  test("Mobile responsive layout", async ({ page }) => {
    ...
    const style = await actions.evaluate(
