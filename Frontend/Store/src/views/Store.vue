<template>
  <div class="store-container">
    ...
    <div v-if="error && !loading" class="error-container">
      <p class="error-message">❌ {{ $t('store.error') }} {{ error }}</p>
      <button @click="loadProducts" class="retry-button">{{ $t('store.retry') }}</button>
    </div>

    <!-- Products Grid -->
    <div v-if="!loading && !error" class="products-grid">
      <div v-if="filteredProducts.length === 0" class="no-products">
        <p>{{ $t('store.noProducts.title') }}</p>
        <p v-if="searchQuery" class="suggestion">
          {{ $t('store.noProducts.suggestion') }}
        </p>
      </div>
    ...
    <div v-if="!loading && filteredProducts.length > 0" class="pagination">
      ...
      <div class="pagination-info">
        {{ $t('store.paginationInfo', { current: currentPage, total: totalPages }) }}
      </div>
