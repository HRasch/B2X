...
export const shopApi = {
  ...
  // Pricing Rules
  async getPricingRules(pagination?: PaginationParams): Promise<PaginatedResponse<PricingRule>> {
    if (DEMO_MODE) {
  ...
  // Discounts
  async getDiscounts(pagination?: PaginationParams): Promise<PaginatedResponse<Discount>> {
    if (DEMO_MODE) {
  ...
  async createDiscount(data: Omit<Discount, 'id' | 'usedCount'>): Promise<Discount> {
    if (DEMO_MODE) {
