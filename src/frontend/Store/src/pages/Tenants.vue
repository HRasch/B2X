<template>
  <div class="tenants">
    ...
    <div v-if="loading" class="loading">{{ $t('ui.loadingTenants') }}</div>
    ...
    <div v-else class="tenants-list">
      <div v-for="tenant in tenants" :key="tenant.id" class="tenant-card">
        ...
        <div class="tenant-actions">
          <button class="btn btn-small">{{ $t('tenants.edit') }}</button>
          <button class="btn btn-small btn-danger">{{ $t('tenants.delete') }}</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="showCreateModal" class="modal">
      <div class="modal-content">
        <h2>{{ $t('tenants.createNewTenant') }}</h2>
        <form @submit.prevent="createTenant">
          ...
          <div class="modal-actions">
            <button type="submit" class="btn btn-primary">{{ $t('tenants.create') }}</button>
