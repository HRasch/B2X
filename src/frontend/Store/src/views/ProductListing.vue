  ...
  <div class="min-h-screen bg-base-100">
    ...
    <header class="bg-primary text-primary-content py-6 px-4">
      <div class="max-w-7xl mx-auto">
        <h1 class="text-3xl font-bold mb-2">{{ $t('productListing.title') }}</h1>
        <p class="text-primary-content/90">{{ $t('productListing.subtitle') }}</p>
      </div>
    </header>
      ...
      <div class="flex flex-col md:flex-row gap-4 mb-8">
        ...
        <div class="flex-1">
          <div class="form-control">
            <label class="label">
              <span class="label-text">{{ $t('productListing.search.label') }}</span>
            </label>
            <input
              v-model="searchQuery"
              type="text"
              :placeholder="$t('productListing.search.placeholder')"
        ...
        <div class="w-full md:w-48">
          <div class="form-control">
            <label class="label">
              <span class="label-text">{{ $t('productListing.sort.label') }}</span>
            </label>
            <select v-model="sortBy" class="select select-bordered" :disabled="loading">
              <option value="name">{{ $t('productListing.sort.options.name') }}</option>
              <option value="price-asc">{{ $t('productListing.sort.options.priceAsc') }}</option>
              <option value="price-desc">{{ $t('productListing.sort.options.priceDesc') }}</option>
              <option value="rating">{{ $t('productListing.sort.options.rating') }}</option>
            </select>
          </div>
        </div>
      </div>
      ...
      <div class="flex flex-col lg:flex-row gap-8">
        ...
        <aside class="lg:w-48 flex-shrink-0">
          <div class="card bg-base-200 shadow-sm">
            <div class="card-body">
              <h3 class="card-title text-base mb-4">{{ $t('productListing.filters.title') }}</h3>

              <!-- Category Filter -->
              <div class="form-control">
                <label class="label">
                  <span class="label-text font-semibold">{{
                    $t('productListing.category.label')
                  }}</span>
                </label>
                <div class="space-y-2">
              ...
              <div class="form-control mt-4">
                <label class="label">
                  <span class="label-text font-semibold">{{
                    $t('productListing.priceRange.label')
                  }}</span>
                </label>
                ...
                <p class="text-xs text-base-content/70 mt-2">
                  {{ $t('productListing.priceRange.placeholder') }}
                </p>
              </div>
              ...
              <div class="form-control mt-4">
                <label class="label cursor-pointer">
                  <span class="label-text">{{ $t('productListing.inStockOnly') }}</span>
        ...
        <main class="flex-1">
          ...
          <div class="mb-6 flex justify-between items-center">
            <div>
              ...
              <p v-if="searchQuery" class="text-sm text-base-content/70">
                {{ $t('productListing.results.foundFor') }}
          ...
          <div v-if="loading" class="flex justify-center items-center py-12">
            <div class="flex flex-col items-center gap-4">
              ...
              <p class="text-base-content/70">{{ $t('productListing.results.loading') }}</p>
            </div>
          </div>
          ...
          <div v-else-if="error" class="alert alert-error mb-6">
            ...
            <button @click="loadProducts" class="btn btn-sm btn-ghost">
              {{ $t('productListing.results.retry') }}
            </button>
          </div>
          ...
          <div v-else class="text-center py-12">
            ...
            <h3 class="text-xl font-semibold mb-2">
              {{ $t('productListing.results.noProducts') }}
            </h3>
            <p class="text-base-content/70 mb-4">
              {{ $t('productListing.results.noProductsMessage') }}
            </p>
            <button
              @click="
                () => {
            ...
            >
              {{ $t('productListing.results.clearFilters') }}
            </button>
          </div>
