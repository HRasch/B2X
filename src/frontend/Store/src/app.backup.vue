          ...
          <ul class="menu menu-horizontal px-1 gap-2">
            ...
            <li>
              <NuxtLink to="/cart" class="btn btn-ghost gap-2">
                {{ $t('navigation.cart') }}
                <span v-if="cartItemsCount > 0" class="badge badge-primary">
                  {{ cartItemsCount }}
                </span>
              </NuxtLink>
            </li>
            ...
            <li v-if="hasAdminAccess">
              <button
                ...
                class="btn btn-ghost"
                :class="{ 'btn-active': isAdminActive }"
...
// Lazy load stores to avoid SSR issues
const authStore = import.meta.client ? useAuthStore() : null;
const cartStore = import.meta.client ? useCartStore() : null;
// const adminStore = import.meta.client ? useAdminStore() : null; // unused

// Import stores for client-side usage
function useAuthStore() {
  return import('~/stores/auth').then(m => m.useAuthStore());
}

function useCartStore() {
  return import('~/stores/cart').then(m => m.useCartStore());
}
...
onMounted(async () => {
  if (import.meta.client) {
    const [auth, cart, admin] = await Promise.all([
      useAuthStore(),
      useCartStore(),
      useAdminStore(),
    ]);
    isAuthenticated.value = auth.isAuthenticated;
    hasAdminAccess.value = auth.hasAdminAccess;
    cartItemsCount.value = cart.items.length;
    isAdminActive.value = admin.isActive;
    ...
    watch(
      () => auth.hasAdminAccess,
      val => (hasAdminAccess.value = val)
    );
    watch(
      () => cart.items.length,
      val => (cartItemsCount.value = val)
    );
    watch(
      () => admin.isActive,
      val => (isAdminActive.value = val)
    );
  }
});
...
const logout = async (): Promise<void> => {
  if (import.meta.client) {
    const admin = await useAdminStore();
    const auth = await import('~/stores/auth').then(m => m.useAuthStore());
    admin.clearEditableElements();
    auth.logout();
 ...
 */
const toggleAdminMode = async (): Promise<void> => {
  if (import.meta.client) {
    const admin = await useAdminStore();
    admin.toggleMode();
    isAdminActive.value = admin.isActive;
