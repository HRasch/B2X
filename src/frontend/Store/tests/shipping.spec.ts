...
test.describe('Shipping Cost Display - P0.6-US-002', () => {
  ...
  test('Accessibility: Shipping method selection with keyboard', async ({ page }) => {
    await page.goto('http://localhost:5173/shop');
    await page.click('.add-to-cart-btn');
    await page.goto('http://localhost:5173/cart');
    ...
    // Tab to first method
    const firstRadio = page.locator(".shipping-option input[type='radio']").first();
    ...
    const secondRadio = page.locator(".shipping-option input[type='radio']").nth(1);
  ...
  test('PAngV Compliance: Shipping visible before checkout', async ({ page }) => {
    await page.goto('http://localhost:5173/shop');
    await page.click('.add-to-cart-btn');
    await page.goto('http://localhost:5173/cart');
  ...
  test('Total calculation includes shipping cost', async ({ page }) => {
    ...
    const shippingText = await page.locator('.shipping-row >> span >> nth=-1').textContent();

    const totalText = await page.locator('.summary-row.total >> span >> nth=-1').textContent();
