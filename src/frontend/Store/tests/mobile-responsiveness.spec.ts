...
test.describe('Mobile Responsiveness - Price Display', () => {
  VIEWPORTS.forEach(({ width, height, name }) => {
    test(`Product card displays correctly on ${name} (${width}x${height})`, async ({ page }) => {
    ...
    test(`Cart displays correctly on ${name} (${width}x${height})`, async ({ page }) => {
      await page.setViewportSize({ width, height });
    ...
    test(`Checkout displays correctly on ${name} (${width}x${height})`, async ({ page }) => {
      await page.setViewportSize({ width, height });
    ...
    test(`All text is readable on ${name} (${width}x${height})`, async ({ page }) => {
      await page.setViewportSize({ width, height });
      await page.goto('http://localhost:5173/cart');
...
test.describe('Tablet to Desktop Transition', () => {
  test('Layout switches to two-column at 768px breakpoint', async ({ page }) => {
