...
test.describe('CSS Smoke Tests - Error Detection', () => {
  test('should not have CSS compilation errors in console', async ({ page }: { page: Page }) => {
  ...
  test('should not have Vite CSS module errors', async ({ page }: { page: Page }) => {
...
test.describe('CSS Smoke Tests - DaisyUI Theme', () => {
  test('should have DaisyUI CSS custom properties defined', async ({ page }: { page: Page }) => {
    await page.goto('/');
    await page.waitForLoadState('networkidle');
    ...
    const daisyProps = await page.evaluate(() => {
      ...
      return {
        ...
        primary: style.getPropertyValue('--p') || style.getPropertyValue('--color-primary'),
        secondary: style.getPropertyValue('--s') || style.getPropertyValue('--color-secondary'),
        base100: style.getPropertyValue('--b1') || style.getPropertyValue('--color-base-100'),
        baseContent:
          style.getPropertyValue('--bc') || style.getPropertyValue('--color-base-content'),
      };
    });

    // At least one DaisyUI color should be defined
    const hasColors = Object.values(daisyProps).some(v => v && v.trim() !== '');
  ...
  test('should apply DaisyUI card styles', async ({ page }: { page: Page }) => {
    ...
    const cardStyles = await page.evaluate(() => {
      ...
      card.innerHTML = '<div class="card-body"><h2 class="card-title">Test</h2></div>';
...
test.describe('CSS Smoke Tests - Tailwind Utilities', () => {
  test('should apply flex utilities correctly', async ({ page }: { page: Page }) => {
    await page.goto('/');
  ...
  test('should apply spacing utilities correctly', async ({ page }: { page: Page }) => {
    await page.goto('/');
  ...
  test('should apply color utilities correctly', async ({ page }: { page: Page }) => {
    await page.goto('/');
  ...
  test('should apply responsive utilities', async ({ page }: { page: Page }) => {
    await page.goto('/');
