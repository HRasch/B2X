...
public static class ValidateErpCommand
{
    ...
    private static async Task ExecuteAsync(string erpType, string tenantId, FileInfo? dataFile, FileInfo? output, bool dataQualityReport, bool includeSql)
    {
        ...
        Environment.ExitCode = results.Any(r => r.Severity >= B2X.Api.Validation.ValidationSeverity.Error) ? 1 : 0;
    }
    ...
    private static async Task WriteValidationReportAsync(IEnumerable<B2X.Api.Validation.ValidationResult> results, FileInfo? output)
    {
        var report = new
        {
            ...
            Errors = results.Where(r => r.Severity >= B2X.Api.Validation.ValidationSeverity.Error).Count(),
            Warnings = results.Where(r => r.Severity == B2X.Api.Validation.ValidationSeverity.Warning).Count(),
            Results = results
        };
    ...
    private static async Task WriteDataQualityReportAsync(
        IEnumerable<B2X.Api.Validation.ValidationResult> results,
        ...
        bool includeSql)
    {
        ...
        var report = new
        {
            ...
            Summary = new
            {
                ...
                Errors = dataQualityIssues.Count(i => i.Severity >= B2X.Api.Validation.ValidationSeverity.Error),
                Warnings = dataQualityIssues.Count(i => i.Severity == B2X.Api.Validation.ValidationSeverity.Warning),
                Critical = dataQualityIssues.Count(i => i.Severity == B2X.Api.Validation.ValidationSeverity.Critical)
            },
    ...
    private static IEnumerable<DataQualityIssue> CreateDataQualityIssues(
        B2X.Api.Validation.ValidationResult result,
    ...
    public class DataQualityIssue
    {
        ...
        public B2X.Api.Validation.ValidationSeverity Severity { get; set; }
