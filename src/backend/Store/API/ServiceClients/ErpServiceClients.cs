...
public class CustomerServiceClient : ICustomerServiceClient
{
    ...
    public async Task<CustomerDto?> GetCustomerByErpIdAsync(string erpCustomerId, Guid tenantId, CancellationToken ct = default)
    {
        try
        {
            ...
            var response = await _httpClient.GetAsync($"/api/customers/erp/{erpCustomerId}", ct).ConfigureAwait(false);
            response.EnsureSuccessStatusCode();
            return await response.Content.ReadFromJsonAsync<CustomerDto>(cancellationToken: ct).ConfigureAwait(false);
        }
...
public class UsageServiceClient : IUsageServiceClient
{
    ...
    public async Task<UsageStatsDto> GetUsageStatsAsync(Guid tenantId, DateTime? from = null, DateTime? to = null, CancellationToken ct = default)
    {
        try
        {
            ...
            var response = await _httpClient.GetAsync($"/api/usage/stats{queryString}", ct).ConfigureAwait(false);
            response.EnsureSuccessStatusCode();
            return await response.Content.ReadFromJsonAsync<UsageStatsDto>(cancellationToken: ct).ConfigureAwait(false) ?? new UsageStatsDto();
        }
        catch (HttpRequestException ex)
        {
...
public class AccessServiceClient : IAccessServiceClient
{
    ...
    public async Task UpdateUserAccessAsync(Guid tenantId, string userId, List<string> permissions, string updatedBy, CancellationToken ct = default)
    {
        try
        {
            ...
            var response = await _httpClient.PutAsJsonAsync($"/api/access/users/{userId}", request, ct).ConfigureAwait(false);
            response.EnsureSuccessStatusCode();
        }
        catch (HttpRequestException ex)
        {
