...
public class TenancyServiceClient : ITenancyServiceClient
{
    ...
    public async Task<TenantDto?> GetTenantByIdAsync(Guid tenantId, CancellationToken ct = default)
    {
        ...
        try
        {
            var response = await _retryPolicy.ExecuteAsync(() =>
                _httpClient.GetAsync($"/api/tenants/{tenantId}", ct)).ConfigureAwait(false);
            ...
            var tenant = await response.Content.ReadFromJsonAsync<TenantDto>(cancellationToken: ct).ConfigureAwait(false);
    ...
    public async Task<TenantDto?> GetTenantByDomainAsync(string domain, CancellationToken ct = default)
    {
        ...
        {
            var response = await _retryPolicy.ExecuteAsync(() =>
                _httpClient.GetAsync($"/api/tenants/by-domain/{Uri.EscapeDataString(normalizedDomain)}", ct)).ConfigureAwait(false);
