...
public class LocalizationServiceClient : ILocalizationServiceClient
{
    ...
    public async Task<Dictionary<string, string>?> GetTranslationsAsync(string locale, Guid tenantId, CancellationToken ct = default)
    {
        try
        {
            ...
            var response = await _httpClient.GetAsync($"/api/translations/{locale}", ct).ConfigureAwait(false);
            response.EnsureSuccessStatusCode();
            return await response.Content.ReadFromJsonAsync<Dictionary<string, string>>(cancellationToken: ct).ConfigureAwait(false);
        }
    ...
    public async Task<string?> GetTranslationAsync(string key, string locale, Guid tenantId, CancellationToken ct = default)
    {
        ...
        {
            _httpClient.DefaultRequestHeaders.Add("X-Tenant-ID", tenantId.ToString());
            var response = await _httpClient.GetAsync($"/api/translations/{locale}/{key}", ct).ConfigureAwait(false);
            response.EnsureSuccessStatusCode();
            var result = await response.Content.ReadFromJsonAsync<TranslationResult>(cancellationToken: ct).ConfigureAwait(false);
            return result?.Value;
        }
        catch (HttpRequestException ex)
        {
