---
docid: BS-011
title: BS RENAME B2CONNECT TO B2XGATE
owner: @DocMaintainer
status: Active
created: 2026-01-08
---

# 🔄 Brainstorm: Rename B2Connect → B2X
...
Renaming **B2Connect** to **B2X** is a comprehensive refactoring task affecting:
...
- `B2Connect.slnx` → `B2X.slnx`
- All 82 `.csproj` files containing `B2Connect`
- `Directory.Build.props` and `Directory.Packages.props`

#### 1.2 Namespaces (CRITICAL)
All namespaces starting with `B2Connect.` must change to `B2X.`:
```csharp
// Before
namespace B2Connect.Catalog.Domain.Models;
...
- `B2Connect.Identity.*`
- `B2Connect.Legal.*`
- `B2Connect.Localization.*`
- `B2Connect.Orders.*`
- `B2Connect.PatternAnalysis.*`
- `B2Connect.Returns.*`
- `B2Connect.SearchService.*`
- `B2Connect.ServiceDefaults.*`
- `B2Connect.Shared.*`
- `B2Connect.SmartDataIntegration.*`
- `B2Connect.Tenancy.*`
- `B2Connect.Theming.*`
- `B2Connect.Tools.*`
- `B2Connect.Utils.*`
...
All `using B2Connect.*;` statements across the codebase.
...
|--------|-----------------|
| B2ConnectMCP | Complete rename → B2XGateMCP |
...
- Container names: `b2connect-*` → `b2xgate-*`
- Image names: `b2connect/*` → `b2xgate/*`
- Volume names: `b2connect-*` → `b2xgate-*`
- Network names: `b2connect-*` → `b2xgate-*`

#### 4.2 Kubernetes
- Namespace: `b2connect` → `b2xgate`
- Service labels
- ConfigMaps
- Deployments
...
All shell scripts containing `B2Connect` or `b2connect`:
- `scripts/aspire-start.sh`
- `scripts/aspire-stop.sh`
- `scripts/check-ports.sh`
- `scripts/deployment-status.sh`
- `scripts/b2connect-heartbeat.service` → `b2xgate-heartbeat.service`
- `scripts/b2connect-heartbeat.timer` → `b2xgate-heartbeat.timer`
...
- Folder rename: `B2Connect/` → `B2XGate/`
...
1. **Create comprehensive backup**
   - Git branch: `feature/rename-b2connect-to-b2xgate`
   - Export current state

2. **Generate complete file inventory**
   - List all files containing `B2Connect` (case-insensitive)
...
2. **Shared libraries**
   - All `B2Connect.Shared.*` projects

3. **CLI projects**
   - `B2Connect.CLI.*` → `B2XGate.CLI.*`
...
6. **Search for remaining references**
   ```bash
   grep -ri "b2connect" --include="*.cs" --include="*.ts" --include="*.json"
   ```
...
```powershell
# B2Connect-to-B2XGate-rename.ps1
...
#!/bin/bash
# verify-rename.sh
echo "Checking for remaining B2Connect references..."
find . -type f \( -name "*.cs" -o -name "*.csproj" -o -name "*.json" \) \
    -exec grep -l -i "b2connect" {} \;
```
