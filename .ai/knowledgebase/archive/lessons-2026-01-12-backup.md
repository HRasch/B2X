...
8. **Migration path**: For production projects on Aspire 8.x, first upgrade to 9.x, then to 13.x (per Microsoft docs)
...
The `AssetTargetFallback` MSBuild property allowed NuGet _restore_ to succeed, but .NET runtime **cannot load net8.0 assemblies** into a net10.0 application without explicit configuration.
...
- GitHub PR [#12500](https://github.com/dotnet/aspire/pull/12500): "Target net10.0 in client integrations" - Added net10.0 to _client_ packages, but NOT hosting packages
...
- **SA1210/SA1208**: Order using directives alphabetically (System._ first, then Microsoft._, then project namespaces)
...
## Session: 12. Januar 2026 - JSON Circular Reference in EF Core Entity Navigation Properties

### Order ↔ OrderItem Circular Reference Causes Serialization Failure

**Issue**: Creating an order via POST API returned HTTP 500 with error:

```
System.Text.Json.JsonException: A possible object cycle was detected.
This can either be due to a cycle or if the object depth is larger than
the maximum allowed depth of 32. Consider using ReferenceHandler.Preserve
on JsonSerializerOptions to support cycles.
Path: $.Items.Order.Items.Order.Items.Order...
```

**Root Cause**: EF Core entities have bidirectional navigation properties:
...
When serializing `Order`, System.Text.Json follows `Items` → `OrderItem.Order` → `Order.Items` → infinite loop.

**Solution**: Configure `ReferenceHandler.IgnoreCycles` in `Program.cs`:
...
| `ReferenceHandler.Preserve` | Preserves references with `$id`/`$ref` | Adds metadata to JSON |
| `[JsonIgnore]` on navigation | Explicit control | Must add to every back-reference |
| DTOs without navigation | Clean separation | More code to maintain |

**Best Practice**: Use DTOs for API responses instead of exposing EF entities directly.

**Key Lessons**:

1. **EF Core navigation properties are dangerous for JSON** - bidirectional refs cause cycles
2. **System.Text.Json has no default cycle handling** - unlike Newtonsoft.Json
3. **`ReferenceHandler.IgnoreCycles`** is the simplest fix for existing codebases
4. **DTOs are the proper solution** for API boundaries

---

## Session: 12. Januar 2026 - PowerShell Background Jobs for API Integration Testing

### Terminal Session Conflicts When Testing APIs

**Issue**: Running `dotnet run` then `curl`/`Invoke-WebRequest` in the same VS Code terminal caused the server to terminate immediately after the HTTP request.

**Root Cause**: VS Code terminal executes commands sequentially. When the terminal runs `dotnet run`, it blocks. Any attempt to run another command requires terminating the blocking process.

**Solution**: Use PowerShell `Start-Job` to run the server as a background job:
...
Stop-Job -Name ApiServer          # Stop running job
Remove-Job -Name ApiServer        # Clean up completed job
```

**Alternative Approaches**:
| Method | Use Case | Pros | Cons |
|--------|----------|------|------|
| `Start-Job` | Quick testing | Simple, built-in | No live output |
| `Start-Process` | Interactive | New window | Manual cleanup |
| VS Code Task | Recurring | Reproducible | Configuration needed |
| Docker | CI/CD | Isolated | Setup overhead |

**Key Lessons**:

1. **PowerShell jobs are stateless** - environment variables must be set inside the script block
2. **Allow sufficient startup time** - 10-15 seconds for .NET + EF Core + Wolverine
3. **Always clean up jobs** - `Stop-Job` + `Remove-Job` prevent orphaned processes
4. **Check job output on failure** - `Receive-Job -Keep` shows build/runtime errors

---

## Session: 12. Januar 2026 - In-Memory Database Provider Doesn't Support Migrations

### MigrateAsync Fails with In-Memory EF Core Provider

**Issue**: API startup failed with error when using in-memory database:
...
**Root Cause**: `context.Database.MigrateAsync()` is a **relational-only operation**. The in-memory provider doesn't support migrations because it has no schema to migrate.

**Solution**: Add conditional check before calling migrations:
...
**Alternative - EnsureCreated for Testing**:
...
| Schema Updates | N/A | Migrations | Migrations |

**Key Lessons**:

1. **In-memory is NOT a database** - it's just dictionaries in memory
2. **Always check provider type** before calling relational methods
3. **Use `EnsureCreatedAsync()`** for in-memory testing
4. **Configuration-driven providers** allow flexible testing vs. production
