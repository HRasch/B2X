...
| **Nuxt 2/Bridge Support** | Partial in @nuxt/kit | Completely removed | N/A for B2X |
...
### B2X Specific Plugin Updates Required
...
⚠️ **B2X Impact**: Verify CI/CD pipelines and development environments use Node.js 20.x or later.
...
- [ ] Review breaking changes relevant to B2X
...
### Recommended Migration Path for B2X

1. **Keep `srcDir: 'src'`**: B2X already uses a custom srcDir, so the new `app/` default won't apply
...
| Module load order changes | Low | Unlikely to affect B2X (single layer) |
...
2. **Plugin Compatibility**: All B2X plugins have Nuxt 4-compatible versions
...
6. **Existing Structure**: B2X's custom `srcDir: 'src'` can be maintained
