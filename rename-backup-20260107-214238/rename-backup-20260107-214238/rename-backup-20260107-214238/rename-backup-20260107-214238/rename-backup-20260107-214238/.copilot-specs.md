# GitHub Copilot Specifications for B2X

## Overview
This document defines guidelines and specifications for using GitHub Copilot in the B2X project. It ensures consistency, quality, and best practices across AI-assisted development.
...
- Namespace hierarchy: `B2X.[ServiceName].[Feature]`
...
### 3.3 Exception Handling - Result-Pattern Approach

**Policy: Avoid Exceptions for Flow Control - Use Result-Pattern Instead**

We use the **Result-Pattern** for explicit error handling:
...
{
    if (!IsValidId(id))
        return new Result<User>.Failure("Invalid user ID");
...
// Caller pattern matches explicitly:
var result = GetUser(42);
var message = result.Match(
    onSuccess: (user, msg) => $"✅ {msg}",
    onFailure: (error, ex) => $"❌ {error}"
);
```
...
- ✅ Clear success/failure paths (no implicit throws)
- ✅ Easier to test and reason about
- ✅ Type-safe error handling

**Guidelines:**
- Use `Result<T>` for operations that can fail
- Use `Result.Match()` or `Result.Fold()` for handling outcomes
- **Never** use exceptions for normal flow control
- Only catch exceptions for truly exceptional cases (system errors, fatal conditions)
- Preserve exception context when converting to `Result.Failure`
- Log both success and failure paths for debugging

### 3.3a Type-Safe Error Codes (CRITICAL)

**POLICY: Never use magic strings for error codes - Use ErrorCodes class**

```csharp
// ❌ WRONG: Magic strings
return new Result<T>.Failure("InvalidCredentials", "Invalid credentials");
return new Result<T>.Failure("UserNotFound", "User not found");

// ✅ CORRECT: Type-safe ErrorCodes class
...
- All error codes are defined in `backend/shared/types/ErrorCodes.cs`
- Use constants like `ErrorCodes.InvalidCredentials`, `ErrorCodes.NotFound`
- Call `.ToMessage()` for human-readable error messages
- Call `.GetStatusCode()` for automatic HTTP status code mapping
- No magic strings anywhere in the codebase

**Benefits:**
- ✅ Compile-time type safety (no typos)
- ✅ IntelliSense support for all error codes
- ✅ Easy refactoring (rename once, updates everywhere)
...
return result.Match(
    onSuccess: (data, msg) => Ok(new { data, message = msg }),
    onFailure: (code, msg) => {
...
**References:**
- [CODING_STANDARDS.md](CODING_STANDARDS.md) - Section 2: Error Handling & Result-Pattern
- [ERROR_CODES_TYPE_SAFE.md](ERROR_CODES_TYPE_SAFE.md) - Detailed error code system
- [RESULT_PATTERN_GUIDE.md](RESULT_PATTERN_GUIDE.md) - Complete implementation guide
...
- **AppHost Project** (`*.AppHost`): Custom orchestration application using .NET Process management
  - Simplified approach without requiring Aspire.Hosting framework dependencies (e.g., DCP)
  - Uses `System.Diagnostics.Process` for explicit service lifecycle management
  - Defines all service paths, ports, and configurations in a single location
  - Highly transparent and debuggable - you can see exactly what's happening
  - Works cross-platform without Docker dependencies
...
- Use custom AppHost with explicit `System.Diagnostics.Process` for service orchestration
- Define all services with names, paths, and ports in a centralized configuration
- Use Serilog for structured logging and service startup/shutdown events
- Implement health checks with `MapHealthChecks()` in Program.cs for each service
- Service discovery via hardcoded localhost addresses (inmemory development)
- Configure resilience via Polly in individual services
- No Aspire Dashboard dependency - instead use service startup logging and manual health checks
- All services run as managed child processes started sequentially with delays
...
## 23. Documentation & Project Organization

### 23.1 Documentation Structure

**Goal**: Maintain a clean, organized project with clear documentation hierarchy to minimize clutter and confusion.

**Master Documentation (Root Level - 4 files):**
- `README.md` - Project overview, quick start
- `GETTING_STARTED.md` - First-time setup and development environment
- `DEVELOPMENT.md` - Development workflow and guidelines
- `BUSINESS_REQUIREMENTS.md` - Features and roadmap

**Feature & Tool Documentation (docs/ subdirectories):**

`docs/architecture/`
- `ASPIRE_GUIDE.md` - .NET Aspire/AppHost orchestration and setup
- `VSCODE_ASPIRE_CONFIG.md` - VS Code debug configuration

`docs/features/` (One file per feature)
- `CATALOG_IMPLEMENTATION.md` - Catalog Service API, endpoints, models
- `AOP_VALIDATION_IMPLEMENTATION.md` - AOP filters, FluentValidation, validators
- `EVENT_VALIDATION_IMPLEMENTATION.md` - Domain event validation system
- `ELASTICSEARCH_IMPLEMENTATION.md` - Search indexing, queries, multilingual
- `LOCALIZATION_IMPLEMENTATION.md` - i18n, entity localization, translations
- `ADMIN_FRONTEND_IMPLEMENTATION.md` - Admin UI, components, forms

`docs/guides/`
- `DEBUG_QUICK_REFERENCE.md` - Quick lookup for debug configs and workflows
- `TESTING_GUIDE.md` - Unit tests, integration tests, E2E tests
- `VERIFICATION.md` - How to verify feature implementations

**Archive (DOCS_ARCHIVE/ folder):**
Outdated status reports and session summaries:
- Historical reports (e.g., FINAL_STATUS_REPORT.md, SESSION_COMPLETE.md)
- Deprecated implementation notes
- Superseded API references

### 23.2 File Organization Hierarchy
...
DOCS_ARCHIVE/                 ← Historical (69 files)
└── *_deprecated.md
    *_complete.md
    etc.
```

### 23.3 Documentation Guidelines

**When to Create/Update Documentation:**

1. **New Feature**: Create ONE `docs/features/FEATURE_NAME_IMPLEMENTATION.md`
   - Include: API endpoints, models, validators, tests
   - Include: Usage examples, configuration
   - Include: Link from GETTING_STARTED.md
   
2. **Bug Fix/Enhancement**: Update existing feature documentation
   - No new files unless it's a complete new feature
   - Update relevant sections inline
   
3. **Status Reports**: Archive after project review
   - Move to `DOCS_ARCHIVE/` with date prefix
   - Example: `DOCS_ARCHIVE/2025-12-26_SESSION_COMPLETE.md`

**Documentation Format for Features:**
```markdown
...
## Architecture
Diagrams, design patterns, key decisions
...
## References
Links to related docs and .copilot-specs.md sections
```

### 23.4 Link Style in Documentation

**In feature files, use relative links to navigate:**

✅ Good:
```markdown
See [ASPIRE_GUIDE.md](../architecture/ASPIRE_GUIDE.md)
See [GETTING_STARTED.md](../../GETTING_STARTED.md)
See [.copilot-specs.md](../../.copilot-specs.md) Section 20
```

❌ Don't use:
```markdown
See ../../../.copilot-specs.md  (too many levels)
See /docs/architecture/...     (absolute paths)
```

### 23.5 Implementation Checklist

When adding a new feature documentation:

```
□ Create docs/features/FEATURE_IMPLEMENTATION.md
□ Add overview, architecture, examples
□ Include configuration and testing sections
□ Update GETTING_STARTED.md with link
□ Update .copilot-specs.md Section 23 if needed
□ Add references to related docs
□ Verify all internal links work
□ Git commit with clear message
```

### 23.6 Why This Structure?

**Root (4 files):**
- Minimal clutter, GitHub-friendly
- Clear entry points for new developers
- GETTING_STARTED.md acts as hub

**docs/architecture/:**
- System design and debugging tools
- Separate from feature implementations
- Guides developers to right tools

**docs/features/:**
- One clear file per feature
- Easy to find, easy to update
- Reduces maintenance burden

**docs/guides/:**
- How-to's and reference materials
- Quick lookups, testing guides
- Verification procedures

**DOCS_ARCHIVE/:**
- Old status reports not needed in active development
- Preserved for historical reference
- Not cluttering main workspace

### 23.7 Maintenance Rules

**DO:**
- Keep root at 4 essential files only
- Create new features in `docs/features/`
- Archive old status reports to `DOCS_ARCHIVE/`
- Update links when moving files
- Reference related docs

**DON'T:**
- Add `.md` files to root (except the 4 core)
- Create duplicate feature documentation
- Leave broken links
- Mix old and new docs
- Forget to update GETTING_STARTED.md
...
**Maintainer**: B2X Team
**Latest Changes**: Added Section 24 - Cross-Platform Bash Guidelines

## 24. Cross-Platform Bash Script Guidelines
...
```bash
# ✅ CORRECT: Portable shebang
#!/usr/bin/env bash

# ❌ WRONG: Platform-specific paths
#!/bin/bash           # May not exist on some systems
#!/usr/bin/bash       # Hardcoded path breaks on macOS ARM64
```

**Rationale**: `#!/usr/bin/env bash` works on all platforms by finding bash in the system PATH.

### 24.2 Error Handling & Strict Mode

**REQUIREMENT**: All scripts must use strict mode for predictable behavior.
...
# -u          Exit if undefined variable is referenced (prevents typos)
# -o pipefail Exit if any command in a pipeline fails
```

### 24.3 Path Handling

**REQUIREMENT**: Use portable path construction that works on Windows (Git Bash, WSL2) and Unix-like systems.

```bash
# ✅ CORRECT: Portable path handling
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
CONFIG_FILE="$PROJECT_ROOT/config.json"

# Use $() instead of backticks (better nesting, more portable)
CURRENT_DIR="$(pwd)"

# ❌ WRONG: Hardcoded paths
CONFIG="/home/user/project/config.json"        # Fails on Windows, macOS
CONFIG="/Users/developer/B2X/config.json" # Fails on other users
CONFIG="C:\Users\Holge\repos\B2X\..."     # Fails on Linux, macOS
```

**For Windows Compatibility**:
- Never use backslashes in paths: use `$PWD` and `/` instead
- Git Bash, WSL2, and MSYS2 all convert `/` paths correctly
- Use `pwd` to get current directory (works on all platforms)

### 24.4 Conditional Logic for Platform Detection

**WHEN NEEDED**: Detect platform only for truly platform-specific operations.
...
**BEST PRACTICE**: Avoid OS-specific code when possible. Use portable tools:
- `command -v` instead of `which` (more portable)
- `nc` or `/dev/tcp` for port checking (instead of platform-specific tools)
- Standard POSIX utilities (cut, sed, grep, etc.)

### 24.5 Command Availability Checks

**REQUIREMENT**: Always verify required commands exist before using them.
...
**REQUIREMENT**: Always quote variables to handle spaces and special characters.
...
echo $SCRIPT_DIR         # Splits on spaces
```

### 24.7 Array Handling

**REQUIREMENT**: Use arrays for lists to handle special characters safely.
...
**REQUIREMENT**: Provide clear, actionable error messages with appropriate exit codes.
...
build_project() {
    ...
    if ! dotnet build "$project_path"; then
        log_error "Build failed for: $project_path"
        return 1
    fi
    
    log_success "Build completed: $project_path"
}
```

### 24.9 Platform-Specific Features to AVOID

**These should NOT be used in cross-platform scripts**:

```bash
# ❌ WRONG: sed differences between platforms
sed -i .bak 's/old/new/' file  # macOS syntax
sed -i 's/old/new/' file        # Linux syntax

# ❌ WRONG: date formatting (totally different on macOS vs Linux)
date +%s  # Works differently on macOS/Linux

# ❌ WRONG: Unquoted variables with redirects
echo "test" >> $OUTPUT_FILE     # Breaks on spaces
```

### 24.10 Testing Cross-Platform Scripts

**REQUIREMENT**: Test scripts on all three platforms before committing.

- **Windows (Git Bash)**: `./scripts/aspire-start.sh`
- **Windows (WSL2)**: `./scripts/aspire-start.sh`
- **macOS**: `./scripts/aspire-start.sh`
- **Linux**: `./scripts/aspire-start.sh`

All must work identically on all platforms.
...
|---------|---------|----------|
| **Hardcoded paths** | Only works on specific user/OS | Use `$(pwd)` and relative paths |
| **Using backslashes** | Git Bash converts `\` incorrectly | Always use forward slashes `/` |
| **Unquoted variables** | Spaces/special chars break | Always quote: `"$VAR"` |
| **Platform-specific commands** | `sed -i` syntax differs | Test on all platforms |
| **Assuming commands exist** | Cryptic errors when missing | Use `command -v` to check |
| **Missing `set -euo pipefail`** | Errors go undetected | Add at top of every script |
| **Using `which`** | Behaves differently on platforms | Use `command -v` instead |
| **Forgetting to quote `$@`** | Arguments with spaces break | Always use `"$@"` |
