...
**Created**: 2026-01-07  
**Reference**: [ADR-051] Rename B2X to B2X
...
Phase 1 of the B2X → B2X rename initiative.
...
- [ ] Create feature branch `feature/rename-B2X-to-b2x`
- [ ] Generate complete file inventory (all files containing B2X)
...
- [ ] Refactor all `namespace B2X.*` → `namespace B2X.*`
- [ ] Update all `using B2X.*` statements
...
- [ ] No B2X references in .cs files

### References

- [ADR-051] Rename B2X to B2XGate
...
- [ ] Rename all `B2X.Shared.*` projects
- [ ] Rename CLI projects (`B2X.CLI.*` → `B2XGate.CLI.*`)
...
- [ ] Rename B2XMCP → B2XGateMCP (folder, package, code)
...
- [ ] Rename container names: `B2X-*` → `b2xgate-*`
- [ ] Rename image names: `B2X/*` → `b2xgate/*`
- [ ] Rename volume names: `B2X-*` → `b2xgate-*`
- [ ] Rename network names: `B2X-*` → `b2xgate-*`
- [ ] Update Kubernetes namespace: `B2X` → `b2xgate`
...
- [ ] Rename `B2X-heartbeat.service` → `b2xgate-heartbeat.service`
- [ ] Rename `B2X-heartbeat.timer` → `b2xgate-heartbeat.timer`
...
- [ ] No B2X references in documentation
- [ ] All links valid
...
- [ ] Execute grep search for remaining B2X references
...
grep -ri "B2X" --include="*.cs" --include="*.ts" --include="*.json" --include="*.md"
```
