...
function runEslint(projectPath, projectName) {
  try {
    const output = execSync(`cd ${projectPath} && npm run lint:check 2>&1`, {
      encoding: 'utf8',
      maxBuffer: 1024 * 1024 * 10 // 10MB buffer
    });

    return { output, projectName, success: true };
  } catch (error) {
    // ESLint exits with code 1 when it finds issues, but we still want the output
    return { output: error.stdout || error.stderr || error.message, projectName, success: false };
  }
}
...
function parseEslintOutput(output, projectName) {
  ...
  for (const line of lines) {
    // Match ESLint format: /path/to/file.ts:line:col: error/warning message
    const match = line.match(/^([^:]+):(\d+):(\d+):\s*(error|warning)\s+(.+)$/);
    if (match) {
      const [, filePath, , , type, message] = match;
      const file = filePath.trim();

      if (!fileIssues[file]) {
        fileIssues[file] = { errors: 0, warnings: 0, issues: [] };
      }

      if (type === 'error') {
        fileIssues[file].errors++;
      } else {
        fileIssues[file].warnings++;
      }

      fileIssues[file].issues.push({ type, message, line: line.trim() });
    }
  }
